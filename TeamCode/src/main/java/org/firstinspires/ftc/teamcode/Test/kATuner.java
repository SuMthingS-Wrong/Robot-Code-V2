...
public class kATuner extends CommandOpMode {
    private Button set1000, set2000, set5000, coarse, fine, incKA, decKA, extraextrafine, set4000, set3000;
    GamepadEx driverOp;
    TelemetryData telemetryData;

    kASubsystem m_subsystem;
    int[] rpms = {1000,2000, 3000, 4000, 5000};
    public void initialize(){
        super.reset();
        telemetryData = new TelemetryData(telemetry);
        driverOp = new GamepadEx(gamepad1);
        m_subsystem = new kASubsystem(hardwareMap);
        ...
        incKA = (new GamepadButton(driverOp, GamepadKeys.Button.LEFT_BUMPER))
                .whenHeld(new InstantCommand(m_subsystem::incKA, m_subsystem));
        incKA = (new GamepadButton(driverOp, GamepadKeys.Button.LEFT_BUMPER))
                .whenHeld(new InstantCommand(m_subsystem::decKA, m_subsystem));
    }

    @Override
    public void run() {
        super.run();
        telemetryData.addData("kS", m_subsystem.getKA());
